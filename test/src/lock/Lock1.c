...
static void *thread_func(void *args) {
  ...
  for (int i = 0; i < ITERATIONS; ++i) {
    ...
    if (i % 1000 == 0)
      _SIRIUS_CPU_RELAX(); // Borrow the macro inside the header file.
  }
...
static inline bool test_multi_thread_contention() {
  ...
  sirius_infosp("Operation " log_purple "%.0f" log_green " times per second\n",
                expected / elapsed);
...
#  endif
  ...
  lock_lock(lock);
  sirius_log_writesp(sirius_log_level_info, log_purple,
...
static void *thread_func_fairness(void *args) {
  int thread_index = *(int *)args;
  int count = 0;
  ...
  sirius_infosp("The thread executed " log_purple "%d" log_green
