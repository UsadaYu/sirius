...
static void _init() {
  ...
  if (!_internal_log_init()) exit(EXIT_FAILURE);
  if (!_log_init()) exit(EXIT_FAILURE);
...
__declspec(allocate(".CRT$XCU")) static const void (
    *init_ptr)() = _init;
#pragma section(".CRT$XPD", read)
__declspec(allocate(".CRT$XPD")) static const void (
    *deinit_ptr)() = _deinit;
#endif
