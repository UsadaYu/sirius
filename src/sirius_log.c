...
bool log_init() { return (bool)(!sirius_mutex_init(&g_mutex, nullptr)); }
...
#define P(fd, type)                                                        \
  ...
  va_end(args);                                                            \
  g_size = fd > STDERR_FILENO                                              \
               ? g_size                                                    \
               : g_size + snprintf(g_buf + g_size, sizeof(g_buf) - g_size, \
                                   log_color_none);                        \
...
#define SP(type)                                                              \
  g_size = snprintf(g_buf, sizeof(g_buf), "%s[%02d:%02d:%02d " #type " %s] ", \
...
#define SP(type)                                                             \
  g_size =                                                                   \
      snprintf(g_buf, sizeof(g_buf),                                         \
               "%s[%02d:%02d:%02d " #type " %s %" PRIu64 " %s (%s|%d)] ",    \
               color, g_tm_info.tm_hour, g_tm_info.tm_min, g_tm_info.tm_sec, \
               module, sirius_thread_id, file, func, line);
