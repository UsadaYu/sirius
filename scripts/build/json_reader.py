...
def main():
    ...
    try:
        ...
        if len(sys.argv) == 2:
            print(reader.display_all(), flush=True)
            sys.exit(0)
        ...
        if value is not None:
            print(reader.display_value(value), flush=True)
        else:
            print(
                ANSI_RED
        ...
        print(f"{ANSI_RED}[Error] {e}{ANSI_RESET}", flush=True)
        sys.exit(1)
    except ValueError as e:
        print(f"{ANSI_RED}[Json Error] {e}{ANSI_RESET}", flush=True)
        sys.exit(1)
    except Exception as e:
        print(f"{ANSI_RED}[Exception] {e}{ANSI_RESET}", flush=True)
        sys.exit(1)
