 ...
 *
 * @note The `basename` function provided by posix is not
 *  used here.
 *  The prototype of the basename function is:
...
typedef struct {
  /**
   * @brief File descriptor for error/warn log writing. The
   *  original configuration will not change if the
   *  parameter is null.
   */
  int *fd_err;

  /**
   * @brief File descriptor for info/debug log writing. The
   *  original configuration will not change if the
   *  parameter is null.
   */
  int *fd_out;
...
#define _LOG_WRITESP(level, color, fmt, ...)         \
  do {                                               \
    sirius_logsp(level, color, log_module_name, fmt, \
                 ##__VA_ARGS__);                     \
  } while (0)
...
#define _WARN(fmt, ...)                              \
  _LOG_WRITE(sirius_log_level_warn, log_yellow, fmt, \
             ##__VA_ARGS__)
#define _WARNSP(fmt, ...)                              \
...
#define _INFO(fmt, ...)                             \
  _LOG_WRITE(sirius_log_level_info, log_green, fmt, \
             ##__VA_ARGS__)
#define _INFOSP(fmt, ...)                             \
...
#define _DEBG(fmt, ...)                                  \
  _LOG_WRITE(sirius_log_level_debg, log_color_none, fmt, \
             ##__VA_ARGS__)
#define _DEBGSP(fmt, ...)                             \
