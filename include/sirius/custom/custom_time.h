 ...
 * @brief Time.
 *
 * @note In this header file, the calculation of time
 *  involves the maximum length of the data type.
 *  Therefore, the following points need to be clarified:
 *  - (1) In Windows environment, `DWORD` is usually of
 *  type `unsigned long`.
 */
...
static inline void _custom_usleep(
  ...
  if (remaining_usec > 0) {
    ...
    double remaining_seconds =
        (double)remaining_usec / 1e6;
    LONGLONG target =
        start.QuadPart +
        (LONGLONG)(remaining_seconds * freq.QuadPart);
    ...
    do {
      ...
      /**
       * Make way for CPU time slices to reduce the CPU
       * usage during busy waiting.
       */
      Sleep(0);
    } while (1);
  }
