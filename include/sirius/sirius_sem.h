 ...
 */
sirius_api int sirius_sem_init(sirius_sem_handle *handle, int pshared,
                               unsigned int value);
 ...
 * @return 0 on success, error code otherwise.
 */
sirius_api int sirius_sem_destroy(sirius_sem_handle *handle);

/**
 * @brief If the semaphore's value is greater than zero, then the decrement
 * proceeds, and the function returns immediately. If the semaphore currently
...
/**
 * @brief This function is the same as the function `sirius_sem_wait`, except
 * that if the decrement cannot be immediately performed, then call returns an
 * error (sirius_err_timeout) instead of blocking.
 *
 * @param[in] handle: Semaphore handle.
 *
 * @return
 * - (1) 0 on success;
 *
 * - (2) `sirius_err_timeout` indicates that obtaining semaphore fails;
 *
 * - (3) error code otherwise.
 */
sirius_api int sirius_sem_trywait(sirius_sem_handle *handle);

/**
 * @brief This function is the same as the function `sirius_sem_wait`, except
 * that `milliseconds` specifies a limit on the amount of time that the call
 ...
 * - (3) error code otherwise.
 */
sirius_api int sirius_sem_timedwait(sirius_sem_handle *handle,
...
#ifndef _WIN32
/**
 * @brief Places the current value of the semaphore pointed to sem into the
