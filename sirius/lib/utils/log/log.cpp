...
class LogManager {
 public:
  ...
  LogManager() : is_creator_(false) {
    log_shm_ = std::make_unique<LogShm>();
  }

  ~LogManager() {}

  bool init(bool is_daemon, std::string daemon_arg) {
    ...
    if (!start_daemon() || !wait_for_daemon()) {
      ProcessLock::lock();
      goto label_free5;
    }
  ...
  void produce(void *src, size_t size) {
    if (!thread_running_.load(std::memory_order_relaxed)) [[unlikely]] {
 ...
 private:
  ...
  bool daemon_check() {
    ...
    size_t found = cmd_line.find(daemon_arg_);

    std::cerr << sz_cmd_line << std::endl;
  ...
  bool daemon_main() {
    ...
    // Monitor the `attached_count` within the process lock.
    while (header->attached_count != 0) {
...
static inline bool daemon_check() {
  LPSTR sz_cmd_line = GetCommandLineA();
  std::string cmd_line(sz_cmd_line);
  size_t found = cmd_line.find(g_daemon_arg);
...
extern "C" bool constructor_utils_log() {
  ...
  g_is_daemon = daemon_check();

  g_log_manager = new LogManager();
  if (!g_log_manager->init(g_is_daemon, g_daemon_arg)) {
