...
utils_check_sizeof(ss_cond_t, CONDITION_VARIABLE);
utils_check_alignof(ss_cond_t, CONDITION_VARIABLE);
#else
utils_check_sizeof(ss_cond_t, pthread_cond_t);
utils_check_alignof(ss_cond_t, pthread_cond_t);
#endif

#if defined(_WIN32) || defined(_WIN64)
static inline int
ss_cond_init_impl(ss_cond_t *__restrict cond,
...
static inline int ss_cond_destroy_impl(ss_cond_t *cond) {
  /**
...
static inline int ss_cond_wait_impl(ss_cond_t *__restrict cond,
...
static inline int ss_cond_timedwait_impl(ss_cond_t *__restrict cond,
                                         ss_mutex_t *__restrict mutex,
...
static inline int ss_cond_signal_impl(ss_cond_t *cond) {
  WakeConditionVariable((CONDITION_VARIABLE *)cond);
  return 0;
}

static inline int ss_cond_broadcast_impl(ss_cond_t *cond) {
...
ss_cond_init_impl(ss_cond_t *__restrict cond,
   ...
   */
  if (!type || *type == kSsThreadProcessPrivate)
